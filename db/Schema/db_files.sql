...
BEGIN
    IF NEW.return_date IS NOT NULL AND NEW.due_date < NEW.return_date THEN
        INSERT INTO fines(borrow_id, user_id, amount, status)
        VALUES (NEW.borrow_id, NEW.user_id, 
                EXTRACT(DAY FROM NEW.return_date - NEW.due_date) * 1.00, -- 1 currency per day
                'pending');
    END IF;
    RETURN NEW;
END;
